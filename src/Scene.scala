  ...
  def traceImage(width: Int, height: Int) {
    ...
    for (y <- (0 until height).par) {
      for (x <- (0 until width).par) {
        ...
        /*
         Changing to set need send a message saying "set(...)
         receive should retrieve this case it and pass to the "set" function
         Coordinator.set(x, y, colour)
         */
        val sceneSystem  = ActorSystem("SceneSytem");
        val sceneCreateor = sceneSystem.actorOf(Props[Coordinator], name = "sceneCreator_" + sceneCounter);
        sceneCreateor ! Set(x, y, colour);
        sceneCounter+=1;
